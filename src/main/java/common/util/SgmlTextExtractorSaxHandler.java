...
/**
 * Copyright (C) 2010-* Nicolas Hernandez Licensed to the Apache Software
...
public class SgmlTextExtractorSaxHandler extends DefaultHandler {
    ...
    /**
     * in order not to consider content of some tags whatever the embedding
     * level
     */
    private int notContentTagDepth = 0;
    /**
     * insert whitespace character after extracted content when the last char is
     * not an end of line
     */
    private Boolean insertWSCharAfterLastChar = false;
    /**
     * in order to detect empty elements in endElement method, we check the
     * characterFired variable empty element should not be considered as
     ...
     * des balises dans le XMLDoc Le problème est que lorsque character retourne
     * un saut de ligne le start du character rapporté est initialisé à 0 et
     * l'on perd la position relative au text source (le XMLDoc) Le true est
     * donc à proscrire !!!
     */
    ...
    /**
     * Start Element localName - The local name (without prefix), or the empty
     * string if Namespace processing is not being performed. qName - The
     * qualified name (with prefix), or the empty string if qualified names are
     * not available.
     */
    @Override
    ...
    public void characters(char[] ch, int start, int length) throws SAXException {
        ...
        if (notContentTagDepth == 0) {
            ...
            if (getInsertWSCharAfterLastChar()
                    ...
                    && (!detaggedText.toString().endsWith(System.lineSeparator()))) //|| !detaggedText.substring(detaggedText.length()-1).equalsIgnoreCase(" "))) 
            {
    ...
    /**
     * Associating a SAX event with a document location when attempting to get
     * the begin/end position of start/end element
     *
     */
    @Override
     ...
     * @return true if tag name isn't a content tag name, false otherwise.
     */
